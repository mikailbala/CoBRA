...
with open(input_file, "r") as infile:
    while True:
        ...
        if len(current_line) > 0:
            ...
            # conditionals if input file is a plasmid:
            if "plasmid" in input_file:
                if (al_len >= (node_len / 2)) and (perc_idn > 97.5):
...
# if input is plasmid, write unique nodes
if "plsmd" in input_file:
    out_node_file = input_file.replace("blastn_plsmd.out", "plsmd_nodes.txt")
    with open(out_node_file, "w") as outfile:
        for i in unq_node_names:
            outfile.write(i)
            outfile.write("\n")

# otherwise, calculate and write the alpha nodes in the output file.
else:
    # calculate n50
...
#
# # make an unq list of beta nodes and print them.
